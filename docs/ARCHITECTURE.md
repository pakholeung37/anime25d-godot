...
| `Core/Animation` | Independent gaze, idle, random, talk and blink state machines |
...
| `Rendering` | Godot resource ownership, GPU bindings and desktop input adapter |
...
Target parameters → gaze → idle → random → talk → blink → smoothing → breath/iris bounce → springs → visibility → rendering.
