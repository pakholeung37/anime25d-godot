...
public sealed class SampleBehavior : ModelBehavior
{
    ...
    public override void PreparePose(ParameterSet pose, double deltaSeconds, bool advance)
    {
        if (advance)
        {
    ...
    public override void ResolvePose(ParameterSet pose, double deltaSeconds, bool advance)
    {
    ...
    public override void DeformCpu(int layer, ReadOnlyPose pose, ReadOnlySpan<float> rest, Span<float> output)
    {
        ...
        var input = new DeformationInput(Definition.Anchors, deformationFrame, Profile.Deformation, Physics.Chest.Displacement, AutomaticMotion.Physics);
        RigDeformer.Deform(Parts[layer], input, output);
    }
}
