...
public partial class RenderChecks : Node
{
    ...
    private void Require(bool condition, string label) { if (!condition) throw new Exception(label); GD.Print("PASS " + label); }
    ...
    private async Task Run()
    {
        ...
        foreach (var entry in models)
        {
            ...
            var viewport = new SubViewport { Size = new Vector2I(definition.Canvas.W, definition.Canvas.H), TransparentBg = true, Disable3D = true, RenderTargetUpdateMode = SubViewport.UpdateMode.Always };
            AddChild(viewport);
            var actor = new AnimeRigNode { AutomaticProcessing = false }; viewport.AddChild(actor);
            foreach (var test in entry.GetProperty("cases").EnumerateArray())
            {
                string testName = test.GetProperty("name").GetString()!;
                if (!new[] { "neutral", "turn", "closed", "wink", "crossfade", "reordered", "animation60" }.Contains(testName)) continue;
                actor.LoadModel(model, Seeded()); var sim = actor.Simulation!;
                ...
                if (!animated) sim.Auto.DisableAll();
                if (testName == "reordered") foreach (var part in sim.Parts) part.DrawOrder = sim.Parts.Length - part.DrawOrder;
                int frames = test.GetProperty("frames").GetInt32();
                if (frames == 0) actor.Advance(0); else for (int i = 0; i < frames; i++) actor.Advance(1.0 / 60);
                await DrawFrames();
                ...
                Require(result == Error.Ok, name + "/" + testName + " rendered"); captures++;
                Require(image.GetPixel(0, 0).A == 0, name + "/" + testName + " transparent corner");
                int visiblePixels = 0; var data = image.GetData();
                for (int i = 3; i < data.Length; i += 4) if (data[i] > 0) visiblePixels++;
            ...
            actor.LoadModel(model, Seeded()); actor.Simulation!.Auto.DisableAll(); actor.Advance(0);
            foreach (var p in actor.Simulation.Parts) if (p.BaseName == "eyewhite") p.Visible = false;
            actor.RefreshPose(); await DrawFrames();
            using var hiddenWhite = viewport.GetTexture().GetImage();
            foreach (var p in actor.Simulation.Parts) if (p.BaseName == "irides") p.Visible = false;
            actor.RefreshPose(); await DrawFrames();
            ...
            var other = new AnimeRigNode { AutomaticProcessing = false }; viewport.AddChild(other); other.LoadModel(model, Seeded());
            var frozenPositions = other.Simulation!.Parts[0].Positions.ToArray();
            actor.Simulation.SetParameter(Parameter.angleX, 1, true); actor.Advance(0.05);
            Require(other.Simulation.Parts[0].Positions.SequenceEqual(frozenPositions), name + " independent instance geometry");
            for (int i = 0; i < 8; i++) actor.LoadModel(model, Seeded());
            other.Free(); actor.ClearModel(); await DrawFrames();
        ...
        System.IO.File.WriteAllText(System.IO.Path.Combine(artifactRoot, "render-tests.json"), JsonSerializer.Serialize(new { status = "passed", captures, renderer = RenderingServer.GetCurrentRenderingMethod() }));
